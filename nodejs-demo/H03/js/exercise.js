...
http.createServer(function (req, res) {
    ...
    /**页面中访问的 .css, .js, .jpg 资源路径 */
    else if (pathName === '/login_bg.jpg') {
        var htmlPath = path.join(__dirname, '../login_bg.jpg');
        loadImg(res, htmlPath);
    }
    else if (pathName.indexOf('/css/') === 0) {
        var htmlPath = path.join(__dirname, '..' + pathName);
        loadCss(res, htmlPath);
    }
    else if (pathName === '/js/baiduTemplate.js') {
        var htmlPath = path.join(__dirname, '../js/baiduTemplate.js');
        loadJs(res, htmlPath);
    }
    ...
    /**交互数据 */
        ...
        for (var k in chapterList) {
            if (chapterList[k].chapterId == chapterId) {
                dataList.push(chapterList[k]);
            }
        }
        ...
        req.on("end", function () {
            ...
            for(var k in userList){
                if (userList[k].username === username && userList[k].pwd === pwd) {
                    data = 1;
                }
            }
            res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        req.on("end", function () {
            ...
            var addObj = queryString.parse(postData);
            // console.log(addObj);
            ...
            var addChapter = {
                ...
                "publishTimer": year + '-' + month + '-' + date,
                "author": "admin",
                "views": 102
            }
            chapterList.push(addChapter);
            res.end();
        });
    }
...
/**将要加载的路径文件返回至页面 */
function loadHtml(res, htmlPath) {
    var fileContent = fs.readFileSync(htmlPath);
    res.writeHead(200, { "Content-Type": "text/html" });
    res.write(fileContent);
    res.end();
}
function loadCss(res, htmlPath) {
    var fileContent = fs.readFileSync(htmlPath);
    res.writeHead(200, { "Content-Type": "text/css" });
    res.write(fileContent);
    res.end();
}
function loadJs(res, htmlPath) {
    var fileContent = fs.readFileSync(htmlPath);
    res.writeHead(200, { "Content-Type": "text/js" });
    res.write(fileContent);
    res.end();
}
function loadImg(res, htmlPath) {
    var fileContent = fs.readFileSync(htmlPath);
